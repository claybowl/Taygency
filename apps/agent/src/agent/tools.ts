...
export class ToolExecutor {
    ...
    return [
      {
        ...
        input_schema: {
          ...
          properties: {
            ...
            duration: { type: 'string', description: 'Estimated duration (e.g., 30m, 2h)' },
          },
          required: ['title'],
        },
      },
      ...
      },
      {
        ...
        input_schema: {
          ...
          properties: {
            ...
            category: { type: 'string' },
            status: { type: 'string', enum: ['active', 'completed', 'someday'] },
            due: { type: 'string' },
            notes: { type: 'string' },
          },
          required: ['taskId'],
      ...
      },
      {
        ...
        input_schema: {
          ...
          properties: {
            skillName: {
              type: 'string',
              description: 'Name of skill to execute (categorize, prioritize, daily-planning)',
            },
            params: {
              type: 'object',
  ...
  async execute(name: string, input: Record<string, unknown>): Promise<ToolResult> {
    switch (name) {
      ...
      case 'execute_skill':
        return this.executeSkill(input.skillName as string, input.params as Record<string, unknown>);
      default:
  ...
  private async updateTask(taskId: string, updates: Record<string, unknown>): Promise<ToolResult> {
  ...
  private async executeSkill(
    ...
    return {
      result,
      action: { type: 'skill_executed', skill: skillName, result: result.output },
    };
  }
}
